...
@app.route('/predict/<user_id>', defaults={'recom_count': 5})
...
@app.route('/predict_file', defaults={'recom_count': 5})
...
def predict_file(recom_count):
    ...
    model.model_data.cate_enc_dict['itemid'] = model.load_models('cate_enc_dict_itemid')
    model.load_matrices('feature')
    final_result = model.predict_file(recom_count,model.model_without_items)
    print(final_result)
    return "Success"
...
if __name__ == "__main__":
    ...
    app.run(port=8080)
    # train(model)
    # predict_with_item(model, [393706],[359990])